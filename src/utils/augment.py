...
def add_noise(input_arr):
    ...
    """
    noise_stdev = random.uniform(1e-3, 5e-3)
    return input_arr + np.random.normal(0, noise_stdev, input_arr.size)


def mask_freq_band(input_arr):
    """
    Randomly removes freqency band
    from input array using band stop filter
    """
    mask_size = random.uniform(2000, 4000)
    mask_start = random.uniform(500, 2500)
